...
from pymillis import MSError, parse_strict

# parse_strict() is deprecated as of 2.0.0; the warning is asserted once in
# TestParseStrictDeprecation and muted for the behavioural tests below.
pytestmark = pytest.mark.filterwarnings("ignore::DeprecationWarning")


class TestParseStrictDeprecation:
    """Test parse_strict() deprecation"""

    def test_should_warn_that_parse_strict_is_deprecated(self):
        """should emit a DeprecationWarning pointing at parse()"""
        with pytest.warns(DeprecationWarning, match="use parse\\(\\) instead"):
